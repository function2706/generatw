...
@dataclass
class UnconfirmedChunk:
    """
    未確定状態を含めたトークンチャンク定義クラス\n
    ...
    **制限事項**\n
    ...
    Attributes:
        ...
        chunk_key (str): confirm 時に用いるオブジェクト固有の文字列
    """
    ...
    def make(cls, text: str):
        ...
        chunk_key = "|".join(",".join(tok.token for tok in toks) for toks, _ in opts)
        return cls(token_opts=opts, prob_total=total, chunk_key=chunk_key)
    ...
    def confirm(self, seed: str) -> list[Token]:
        ...
        # chunk_key はオブジェクトごとに異なる値を算出するために使用
        h = int(sha256(f"{seed}|{self.chunk_key}".encode()).hexdigest(), 16)
