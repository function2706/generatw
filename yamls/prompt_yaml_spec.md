...
`<>` を用いることで, 複数のトークン候補から1つを確率的に選択できる.
選択は実行時に決定論的なシードによって行われ, 同一シードに対しては常に同じ結果を返す.

#### 6.4.1 基本形式

`<` と `>` で候補を囲み, `|` で区切る. 重みを省略した場合は `1.0` として扱われる.

```yaml
# 等確率で a, b, c のいずれか1つが選ばれる
positive: <a|b|c>

# 重み付き: a が選ばれる確率は b の2倍
positive: <2::a|b>
```

重みは相対値であり, 合計が1.0である必要はない.

#### 6.4.2 カンマ連結との組み合わせ

`<>` はカンマ区切りの列中に混在させることができる.
この場合, 各カンマ区切り要素は連結されたトークン列として展開される(直積).

```yaml
# a と (b または c) の組み合わせ: [a,b] または [a,c] が等確率で選ばれる
positive: a,<b|c>

# a と (b,d または c,e) の組み合わせ
positive: a,<b,d|c,e>
```

`<>` 内のカンマはその選択肢内でのトークン連結として解釈される.

```yaml
# [t1,t2,t3] または [t1,t5] のいずれかが選ばれる
# ただし内部の重みに従い, 前者が 6/41, 後者が 35/41 の確率で選ばれる
positive: t1,<2::t2,<3::t3|4::t4>|5::t5>
```

#### 6.4.3 ネスト

`<>` は入れ子にできる. 内側の選択肢全体が外側の1つの候補として扱われる.

```yaml
# hoge, fuga, hogefuga の3択
# 重み比は 14:3:4 (合計 21)
positive: <2::hoge|<3::fuga|4::hogefuga>>
```

ネストされた重みの計算は以下の規則に従う.

- 外側の重み `p`, 内側の選択肢の重みが `r`, `s` の場合: `<p::x|q::<r::y|s::z>>` の重み比は `p(r+s) : qr : qs` となる
...
トークン文字列の途中に `<>` を埋め込むことができる.
`<>` の前後の文字列は, 展開後の先頭・末尾トークンにそれぞれ結合される.

```yaml
# x と (y,A または z,B) の先頭結合:
# [xy, A] または [xz, B] が選ばれる
positive: x<y,A|z,B>
...
インライン埋め込みにおいて, prefix/suffix 部分への**重み付けには対応しない**.
重み付けが必要な場合は, リテラル形式で選択肢ごとに記述すること.

```yaml
# NG: <x|y>z の形式では xz と yz に対して重みを指定できない
positive: <2::x|3::y>z  # z は結合されるが重み付け不可

# OK: リテラル形式で記述する
positive: <2::(xz:1.2)|3::(yz:1.5)>
```
...
- `<>` はネストが可能だが, 深さに応じて重み計算が複雑になるため, 可読性のために深さ2程度を目安とすること
- `<>` 内のカンマはトークン連結として解釈されるため, カンマ自体をトークン内に含めることはできない(この制約は通常のトークン記法と共通)
- `<x|y>z` のような後置文字列に重みを付けることはできない. 重み付けが必要な場合は `<(xz:1.2)|(yz:1.5)>` 等のリテラル形式で記述すること
- `<>` 内の重み(`::` 前の数値)は正の値でなければならない
