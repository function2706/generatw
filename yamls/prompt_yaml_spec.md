...
- 1つもマッチしない場合、パース対象外と見なし最後に表示した文字列をそのまま返す
...
- 同じルール ID が複数存在する場合は、その中の最小値がすべてに割り当てられる。
...
- `stable` なトークンは、自身より新しい世代(つまり新たに追加された) `volatile` なトークンよりも
優先的に記述される。
- `stable` トークン同士は引き継ぎの際に `priority` のルールに従う
...
同じルール ID のもので global が1つでも指定されている場合は、それらすべてが global となる。
